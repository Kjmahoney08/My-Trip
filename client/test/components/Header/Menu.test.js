...
describe('Menu', () => {
	...
	test('base: Adds home location', async () => {
		...
		expect(menuProps.distanceActions.refresh).toHaveBeenCalled();
	});
	...
	test('base: Removes all places', async () => {
		...
		expect(menuProps.distanceActions.clear).toHaveBeenCalled();
	});
	...
	/*
	test('nritter8: Toggles Preferences', async () => {
		const menuToggle = screen.getByTestId('menu-toggle');
		await waitFor(() => user.click(menuToggle));
		const preferencesButton = screen.getByTestId('preferences-button');
		user.click(preferencesButton);
		expect(menuProps.toggleClientSettings).toHaveBeenCalled();
	});

	test('nritter8: Toggles Save Trip', async () => {
		const menuToggle = screen.getByTestId('menu-toggle');
		await waitFor(() => user.click(menuToggle));
		const saveTripButton = screen.getByTestId('save-trip-button');
		user.click(saveTripButton);
		expect(menuProps.toggleSaveTrip).toHaveBeenCalled();
	});
	*/
});
