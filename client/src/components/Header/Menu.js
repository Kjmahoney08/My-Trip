...
import { BsFillPeopleFill, BsHddNetworkFill, BsTextRight } from 'react-icons/bs';
...
function buildMenuButtons(props) {
	return [
		...
		new MenuButtonProps('home-button', () => {
			props.placeActions.append(DEFAULT_STARTING_POSITION);
			props.distanceActions.refresh();
		}, <FaHome />, 'Add Home'),
		...
		new MenuButtonProps('preferences-button', props.toggleClientSettings, <BsTextRight />, 'Preferences'),
		new MenuButtonProps('remove-all-button', () => {
			props.placeActions.removeAll();
			props.distanceActions.clear();
		}, <FaTrashAlt />, 'Remove All', props.disableRemoveAll),
