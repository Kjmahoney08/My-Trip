...
public class TestMicroServer {
    ...
    @Test
    @DisplayName("grantcs: Valid distance request responds with 200 status")
    public void testValidDistanceRequest() throws IOException {
        String ValidDistanceRequestJSON = 
            "{\"requestType\":\"distances\", \"earthRadius\":1.0, \"places\": [] }";
        HttpResponse response = postRequest("/api/distances", ValidDistanceRequestJSON);
        assertEquals(200, response.getStatusLine().getStatusCode());
    }

    @Test
    @DisplayName("kjm43: Invalid Distance request with 404 status")
    public void testInvalidDistanceRequest() throws IOException {
        String InvalidDistanceRequestJSON = "{ }";
        HttpResponse response = postRequest("/api/distances", InvalidDistanceRequestJSON);
        assertEquals(400, response.getStatusLine().getStatusCode());
    }

    @Test
    @DisplayName("kjm43: Valid find reuqest with 200 status")
    public void testValidFindRequest() throws IOException {
        String validFindRequestJSON = 
            "{\"requestType\":\"find\", \"match\":\"dave\", \"limit\":1 }";
        HttpResponse response = postRequest("/api/find", validFindRequestJSON);
        assertEquals(200, response.getStatusLine().getStatusCode());
    }

    @Test
    @DisplayName("kjm43: Invalid find request with 404 status")
    public void testInvalidFindRequest() throws IOException {
        String InvalidFindRequestJSON = "{ }";
        HttpResponse response = postRequest("/api/find", InvalidFindRequestJSON);
        assertEquals(400, response.getStatusLine().getStatusCode());
    }
}
